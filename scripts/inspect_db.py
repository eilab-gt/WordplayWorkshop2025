...
@cli.command()
@click.option('--db', type=click.Path(exists=True), default='logs/logging.db', help='SQLite log database')
...
def logs(db, level, limit, errors_only):
    ...
    # Show summary
    stats_text = "\n".join([f"{row['level']}: [bold]{row['count']}[/bold]" for _, row in stats_df.iterrows()])
    panel = Panel(
...
def extraction(csv, stats):
    ...
    total = len(df)
    successful = len(df[df['extraction_status'] == 'success']) if 'extraction_status' in df else 0
...
@click.option('--csv', type=click.Path(exists=True), help='CSV file containing the paper')
def paper(screening_id, csv):
    ...
    if not csv:
        ...
        for loc in locations:
            for csv_file in loc.glob('*.csv'):
                ...
                if 'screening_id' in df.columns and screening_id in df['screening_id'].values:
                    csv = str(csv_file)
                    found = True
                    break
            if found:
    ...
    extraction_fields = [
        ...
        'llm_family', 'llm_role', 'awscale', 'failure_modes'
    ]
