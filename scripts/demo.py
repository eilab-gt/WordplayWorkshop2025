...
def create_demo_config():
    ...
    config = {
        'search': {
            'queries': {
                'demo': '"LLM" AND "wargaming" AND "simulation"'
            },
            'sources': {
...
def create_sample_papers():
    ...
    papers = pd.DataFrame({
        ...
        'source_db': ['arxiv', 'arxiv', 'google_scholar', 'semantic_scholar', 'crossref'],
        'doi': ['10.1234/fake.2024.001', '', '10.1234/fake.2024.002', '10.1234/fake.2023.001', ''],
        'url': [
...
def create_sample_extraction_results():
    ...
    results = pd.DataFrame({
        'screening_id': ['SCREEN_0001', 'SCREEN_0002', 'SCREEN_0003', 'SCREEN_0004', 'SCREEN_0005'],
        'title': [
            'Large Language Models in Strategic Wargaming: A Novel Approach',
            'GPT-4 as Autonomous Agents in Military Simulations',
            'Evaluating LLM Performance in Crisis Management Games',
        ...
        'venue_type': ['conference', 'tech-report', 'journal', 'workshop', 'journal'],
        'game_type': ['matrix', 'digital', 'seminar', 'hybrid', 'matrix'],
        'open_ended': ['yes', 'yes', 'no', 'yes', 'yes'],
        'quantitative': ['yes', 'no', 'yes', 'yes', 'no'],
        'llm_family': ['GPT-4', 'GPT-4', 'Multiple', 'GPT-3.5', 'Custom Transformer'],
        ...
        'failure_modes': ['escalation', 'bias', 'hallucination', 'bias|hallucination|escalation', ''],
        'awscale': [3, 4, 2, 3, 5],
        'code_release': ['github.com/fake/repo1', 'none', 'none', 'github.com/fake/repo4', 'none'],
        'extraction_status': ['success', 'success', 'success', 'success', 'success']
    })
...
def demo(full):
    ...
    # Create directories
    for dir_name in ['demo_data/raw', 'demo_data/processed', 'demo_data/extracted', 
                     'demo_outputs', 'demo_pdfs', 'demo_logs']:
    ...
    stats = {
        ...
        'Conference papers': len(extraction_results[extraction_results['venue_type'] == 'conference']),
        'Journal papers': len(extraction_results[extraction_results['venue_type'] == 'journal']),
        'Open-ended games': len(extraction_results[extraction_results['open_ended'] == 'yes']),
        'Papers with code': len(extraction_results[extraction_results['code_release'] != 'none'])
    }
