...
def generate_paper_stats(df: pd.DataFrame) -> dict:
    ...
    stats = {
        'total_papers': len(df),
        'date_range': {
            'min_year': int(df['year'].min()) if 'year' in df and not df['year'].isna().all() else None,
            'max_year': int(df['year'].max()) if 'year' in df and not df['year'].isna().all() else None
        },
        'sources': {},
        'venues': {},
        'top_authors': {}
...
def generate_extraction_stats(df: pd.DataFrame) -> dict:
    ...
    if 'extraction_status' in df:
        ...
        stats['extraction_success_rate'] = (success_count / len(df) * 100) if len(df) > 0 else 0
    ...
    if 'code_release' in df:
        has_code = df['code_release'].notna() & (df['code_release'] != 'none') & (df['code_release'] != '')
...
@click.option('--extraction', type=click.Path(exists=True), help='Extraction results CSV')
@click.option('--output', type=click.Path(), help='Output JSON file for stats')
@click.option('--format', type=click.Choice(['console', 'json', 'both']), default='console', help='Output format')
def generate_stats(papers, extraction, output, format):
    ...
    if papers:
        ...
        if format in ['console', 'both']:
            ...
            if paper_stats['sources']:
                ...
                for source, count in sorted(paper_stats['sources'].items(), key=lambda x: x[1], reverse=True):
            ...
            if paper_stats['top_authors']:
                ...
                for i, (author, count) in enumerate(list(paper_stats['top_authors'].items())[:10], 1):
    ...
    if extraction:
        ...
        if format in ['console', 'both']:
            ...
            if extraction_stats['failure_modes']:
                ...
                for mode, count in sorted(extraction_stats['failure_modes'].items(), 
            ...
            if extraction_stats['awscale_distribution']:
                ...
                for scale, count in sorted(extraction_stats['awscale_distribution'].items()):
    ...
    if format in ['console', 'both']:
        console.print("\n[bold]Summary:[/bold]")
        if all_stats['papers']:
            console.print(f"  • Collected {all_stats['papers']['total_papers']} papers from {len(all_stats['papers']['sources'])} sources")
        if all_stats['extraction']:
            console.print(f"  • Successfully extracted {all_stats['extraction']['extraction_success_rate']:.0f}% of papers")
            if all_stats['extraction']['failure_modes']:
                console.print(f"  • Identified {len(all_stats['extraction']['failure_modes'])} types of failure modes")
