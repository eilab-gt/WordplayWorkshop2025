...
class Exporter:
    ...
    def _save_data_files(self, df: pd.DataFrame, output_dir: Path):
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            ...
            # Add summary sheet
            summary_data = {
                'Metric': ['Total Papers', 'Papers with PDFs', 'Papers Extracted', 
                          'Papers with Failure Modes', 'Unique LLM Families'],
                'Value': [
                    len(df),
                    len(df[df['pdf_status'].notna() & df['pdf_status'].str.startswith('downloaded')]),
    ...
    def _create_metadata(self, df: pd.DataFrame, output_dir: Path):
        ...
        metadata = {
            ...
            'data_summary': {
                ...
                'sources': df['source_db'].value_counts().to_dict() if 'source_db' in df.columns else {},
                'year_range': {
            ...
            'quality_metrics': {
                'papers_with_abstracts': len(df[df['abstract'].notna() & (df['abstract'] != '')]),
                'papers_with_dois': len(df[df['doi'].notna() & (df['doi'] != '')]),
                'extraction_success_rate': len(df[df['extraction_status'] == 'success']) / len(df) 
                                          if 'extraction_status' in df.columns else 0
            }
        }
...
"""
    ...
    def _upload_to_zenodo(self, archive_path: Path, df: pd.DataFrame):
        ...
        try:
            ...
            base_url = "https://zenodo.org/api" if "sandbox" not in str(self.zenodo_token) else "https://sandbox.zenodo.org/api"
            ...
            if r.status_code != 201:
                logger.error(f"Failed to create Zenodo deposition: {r.status_code} {r.text}")
                return
            ...
            metadata = {
                'metadata': {
                    ...
                    'keywords': ['LLM', 'wargaming', 'systematic review', 'natural language processing'],
                    'communities': [{'identifier': self.zenodo_community}] if self.zenodo_community else [],
                    'language': 'eng',
                    'license': 'cc-by-4.0'
                }
            }
            ...
            if r.status_code == 200:
                ...
                logger.info("Note: Deposition is in draft state. Publish manually via Zenodo website.")
            else:
    ...
    def export_bibtex(self, df: pd.DataFrame, output_path: Optional[Path] = None) -> Path:
        ...
        for idx, paper in df.iterrows():
            ...
            # Create citation key
            first_author = paper['authors'].split(';')[0].split()[-1] if paper.get('authors') else 'Unknown'
