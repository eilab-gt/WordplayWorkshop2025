...
from ..harvesters.base import Paper
...
class Normalizer:
    ...
    def _normalize_authors(self, authors: str) -> str:
        ...
        for author in author_list:
            ...
            if author:
                ...
                author = re.sub(r'\b(Dr|Prof|Professor|Mr|Mrs|Ms|PhD|Ph\.D)\b\.?', '', author)
    ...
    def _deduplicate_by_doi(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_with_doi = df_with_doi.drop_duplicates(subset=['doi_normalized'], keep='first')
    ...
    def _validate_papers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        columns_to_drop = ['title_normalized', 'doi_normalized', 'authors_normalized', 
                          'title_slug', 'content_hash']
    ...
    def _log_statistics(self):
        ...
        logger.info(f"  Total removed: {self.stats['total_input'] - self.stats['total_output']}")
    
    def find_similar_papers(self, df: pd.DataFrame, title: str, threshold: float = 0.8) -> pd.DataFrame:
