...
class PDFFetcher:
    ...
    def fetch_pdfs(self, df: pd.DataFrame, parallel: bool = True) -> pd.DataFrame:
        ...
        if parallel:
            df = self._fetch_parallel(df)
        else:
            df = self._fetch_sequential(df)
    ...
    def _generate_filename(self, paper: pd.Series) -> str:
        ...
        if title:
            ...
            title_slug = '_'.join(''.join(c for c in word if c.isalnum()) for word in title_words)
    ...
    def cleanup_cache(self, keep_days: int = 30):
        ...
        if removed_count > 0:
            logger.info(f"Cleaned up {removed_count} PDFs ({removed_size / 1024 / 1024:.1f}MB)")
    ...
    def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_size_mb': (total_size / len(pdf_files) / 1024 / 1024) if pdf_files else 0
        }