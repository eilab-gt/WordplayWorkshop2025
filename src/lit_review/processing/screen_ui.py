...
class ScreenUI:
    ...
    def prepare_screening_sheet(self, 
                               df: pd.DataFrame,
                               output_path: Optional[Path] = None,
                               include_asreview: bool = False) -> pd.DataFrame:
    ...
    def _sort_for_screening(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        priority_keywords = [
            'wargame', 'war game', 'crisis simulation', 
            'llm', 'large language model', 'gpt', 'claude'
        ]
        
        for keyword in priority_keywords:
            # Check title
            df.loc[df['title'].str.lower().str.contains(keyword, na=False), 'sort_score'] += 2
            # Check abstract
            df.loc[df['abstract'].str.lower().str.contains(keyword, na=False), 'sort_score'] += 1
    ...
    def _add_metadata(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        screening_cols = [
            'screening_id', 'title', 'authors', 'year', 'venue',
            'abstract', 'doi', 'url', 'source_db',
            'include_ta', 'reason_ta', 'notes_ta',
            'include_ft', 'reason_ft', 'notes_ft',
    ...
    def _create_exclusion_reasons(self) -> pd.DataFrame:
        ...
        """
        reasons = [
            {'Code': 'E1', 'Reason': 'Not a wargame/conflict simulation', 'Stage': 'Title/Abstract'},
            {'Code': 'E2', 'Reason': 'No LLM or LLM <100M params', 'Stage': 'Title/Abstract'},
            {'Code': 'E3', 'Reason': 'No natural language interaction', 'Stage': 'Title/Abstract'},
            {'Code': 'E4', 'Reason': 'Not empirical (opinion/editorial)', 'Stage': 'Title/Abstract'},
            {'Code': 'E5', 'Reason': 'Wrong publication type', 'Stage': 'Title/Abstract'},
            {'Code': 'E6', 'Reason': 'Full text not available', 'Stage': 'Full Text'},
            {'Code': 'E7', 'Reason': 'Not in English (no translation)', 'Stage': 'Full Text'},
    ...
    def _prepare_asreview_format(self, df: pd.DataFrame, base_path: Path):
        ...
        try:
            ...
            asreview_df = pd.DataFrame({
                ...
                'doi': df['doi'].fillna(''),
                'url': df['url'].fillna(''),
                'included': df['include_ta'].apply(lambda x: 1 if x == 'yes' else 0 if x == 'no' else -1)
            })
            ...
            logger.info(f"Prepared ASReview data: {len(labeled_df)} labeled, {len(unlabeled_df)} unlabeled")
