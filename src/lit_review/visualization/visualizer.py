...
class Visualizer:
    ...
    def plot_game_types(self, df: pd.DataFrame, save: bool = True) -> Path | None:
        ...
        try:
            ...
            colors = [
                colors_map.get(game_type, "gray") for game_type in game_counts.index
            ]
    ...
    def plot_awscale_distribution(
        ...
        try:
            ...
            # Convert to numeric
            awscale_values = pd.to_numeric(
                awscale_data["awscale"], errors="coerce"
            ).dropna()
    ...
    def plot_failure_modes(self, df: pd.DataFrame, save: bool = True) -> Path | None:
        ...
        try:
            ...
            # Add value labels
            for i, (_bar, count) in enumerate(zip(bars, counts, strict=False)):
    ...
    def plot_llm_families(self, df: pd.DataFrame, save: bool = True) -> Path | None:
        ...
        try:
            ...
            ax.set_ylabel("Number of Papers", fontsize=12)
            ax.set_title(
                "Distribution of LLM Families Used", fontsize=14, fontweight="bold"
            )
    ...
    def plot_game_characteristics(
        ...
        try:
            ...
            len(df)
            oe_yes_q_yes = len(
                df[(df["open_ended"] == "yes") & (df["quantitative"] == "yes")]
            )
            oe_yes_q_no = len(
                df[(df["open_ended"] == "yes") & (df["quantitative"] == "no")]
            )
            oe_no_q_yes = len(
                df[(df["open_ended"] == "no") & (df["quantitative"] == "yes")]
            )
            oe_no_q_no = len(
                df[(df["open_ended"] == "no") & (df["quantitative"] == "no")]
            )
            ...
            ax.bar(
                ...
                both,
                width,
                label="Both Open-ended & Quantitative",
                color="darkgreen",
            )
            ax.bar(
                x,
            ...
            ax.bar(
                ...
                neither,
                width,
                bottom=[both[0] + open_only[0] + quant_only[0]],
                label="Neither",
                color="lightgray",
            )
            ...
            ax.set_ylabel("Number of Papers", fontsize=12)
            ax.set_title(
                "Game Characteristics: Open-ended vs Quantitative",
                fontsize=14,
                fontweight="bold",
            )
            ax.set_xticks([])
            ...
            if total_height > 0:
                ...
                if quant_only[0] > 0:
                    ax.text(
                        0,
                        both[0] + open_only[0] + quant_only[0] / 2,
                ...
                if neither[0] > 0:
                    ax.text(
                        0,
                        both[0] + open_only[0] + quant_only[0] + neither[0] / 2,
            ...
            for modes_str in df["failure_modes"].dropna():
                if modes_str:
                    ...
                    for mode in modes:
                        ...
                        if mode:
                            failure_counts[mode] = failure_counts.get(mode, 0) + 1
            summary["top_failure_modes"] = dict(
        ...
        if "awscale" in df.columns:
            ...
            if len(awscale_values) > 0:
                summary["awscale"] = {
                    ...
                    "distribution": awscale_values.value_counts()
                    .sort_index()
                    .to_dict(),
                }
