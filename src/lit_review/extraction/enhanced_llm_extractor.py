...
class EnhancedLLMExtractor:
    ...
    def _extract_parallel(self, df: pd.DataFrame) -> dict[Any, dict[str, Any]]:
        ...
        results = {}
        max_workers = min(5, self.config.parallel_workers if hasattr(self.config, 'parallel_workers') else 3)
    ...
    def _extract_single_paper(self, paper: pd.Series) -> dict[str, Any]:
        ...
        try:
            ...
            if not content or len(content) < 100:
                logger.warning(f"Insufficient content for paper: {paper.get('title', 'Unknown')[:50]}")
    ...
    def _clean_tex_content(self, tex_content: str) -> str:
        ...
        for pattern, replacement, *flags in patterns_to_simplify:
            tex_content = re.sub(pattern, replacement, tex_content, flags=flags[0] if flags else 0)
    ...
    def _llm_service_extract(self, content: str, paper: pd.Series) -> dict[str, Any] | None:
...
"""
    ...
    def _calculate_confidence(self, extracted: dict[str, Any]) -> float:
        ...
        key_fields = [
            "research_questions", "key_contributions", "simulation_approach",
            "llm_usage", "evaluation_metrics"
        ]
