...
class LLMExtractor:
    ...
    def extract_all(self, 
        ...
        pdf_papers = df[df['pdf_path'].notna() & (df['pdf_path'] != '') & 
                        (df['pdf_status'].str.startswith('downloaded') | 
                         (df['pdf_status'] == 'cached'))].copy()
        ...
        extraction_cols = [
            'venue_type', 'game_type', 'open_ended', 'quantitative',
            'llm_family', 'llm_role', 'eval_metrics', 'failure_modes',
            ...
            with open(pdf_path, 'rb') as f:
                ...
                if doc.info:
                    ...
                    metadata = {
                        ...
                        'subject': info.get('Subject', b'').decode('utf-8', errors='ignore'),
    ...
    def _prepare_context(self, paper: pd.Series, text: str, metadata: Dict[str, Any]) -> str:
...
"""
    ...
    def _llm_extract(self, context: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Process list fields
            if 'failure_modes' in extracted and isinstance(extracted['failure_modes'], list):
...
"""
    ...
    def _calculate_confidence(self, extracted: Dict[str, Any]) -> float:
        ...
        for field in required_fields:
            if field in extracted and extracted[field] and extracted[field] != 'unknown':
                confidence += 0.08
    ...
    def extract_single_pdf(self, pdf_path: str, 
        ...
        paper = pd.Series({
            ...
            'authors': paper_metadata.get('authors', 'Unknown') if paper_metadata else 'Unknown',
            'year': paper_metadata.get('year', 0) if paper_metadata else 0,
            'abstract': paper_metadata.get('abstract', '') if paper_metadata else ''
        })
