...
class Tagger:
    ...
    def tag_papers(self, df: pd.DataFrame, 
                   use_llm_results: bool = True) -> pd.DataFrame:
        ...
        for idx, paper in df.iterrows():
            ...
            # Update DataFrame
            df.at[idx, 'failure_modes'] = '|'.join(sorted(all_modes)) if all_modes else ''
            df.at[idx, 'failure_modes_regex'] = '|'.join(sorted(regex_modes)) if regex_modes else ''
    ...
    def _build_failure_patterns(self) -> Dict[str, List[re.Pattern]]:
        ...
        patterns['factual_error'] = [
            re.compile(r'\bfactual(?:ly)?\s+(?:error|incorrect|wrong)\b', re.IGNORECASE),
        ...
        patterns['jailbreak'] = [
            ...
            re.compile(r'\bbypass\w*\s+(?:safety|security|guardrails)\b', re.IGNORECASE),
    ...
    def _build_metadata_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            ...
            'seminar_game': re.compile(r'\bseminar\s+(?:game|wargame)\b', re.IGNORECASE),
            'digital_game': re.compile(r'\bdigital\s+(?:game|wargame|simulation)\b', re.IGNORECASE),
            ...
            'human_evaluation': re.compile(r'\bhuman\s+(?:evaluation|assessment|rating)\b', re.IGNORECASE),
            
            # Code availability
            'github': re.compile(r'github\.com/[\w-]+/[\w-]+', re.IGNORECASE),
            'code_available': re.compile(r'\bcode\s+(?:is\s+)?available\b', re.IGNORECASE),
    ...
    def _map_to_vocab(self, category: str) -> Optional[str]:
        ...
        for vocab_cat, terms in self.failure_vocab.items():
    ...
    def _tag_metadata(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for key in ['llm_detected', 'game_type_detected', 'metrics_detected', 'code_detected']:
        ...
        for idx, paper in df.iterrows():
            ...
            for model, pattern in self.metadata_patterns.items():
                if model.startswith('gpt') or model in ['claude', 'palm', 'llama', 'bert']:
                    if pattern.search(text):
            ...
            elif self.metadata_patterns['code_available'].search(text) or \
                 self.metadata_patterns['open_source'].search(text):
    ...
    def _log_statistics(self):
        ...
        logger.info(f"  Papers with failure modes: {self.stats['papers_with_failures']}")
        ...
        if self.stats['pattern_matches']:
            ...
            sorted_patterns = sorted(
                self.stats['pattern_matches'].items(), 
                key=lambda x: x[1], 
                reverse=True
            )[:10]
    ...
    def get_failure_mode_summary(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        summary = pd.DataFrame(
            ...
            columns=['failure_mode', 'count']
        ).sort_values('count', ascending=False)
        
        # Add percentage
        total_papers = len(df[df['failure_modes'].notna() & (df['failure_modes'] != '')])
