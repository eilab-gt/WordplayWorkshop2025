...
class TestEnhancedLLMExtractor:
    ...
    def test_extract_all_sequential(self, mock_extract, mock_models, mock_health, 
                                   extractor, sample_df):
    ...
    def test_get_paper_content_pdf_only(self, extractor, tmp_path):
        ...
        with patch.object(extractor, '_extract_pdf_text', return_value="Extracted text"):
    ...
    @patch.object(EnhancedLLMExtractor, '_extract_tex_content', return_value=("", False))
    ...
    def test_assign_awscale(self, extractor):
        ...
        test_cases = [
            ...
            ({"simulation_approach": "unknown", "human_llm_comparison": "human vs llm"}, 4),
            ({}, 3)  # Default
        ]
    ...
    def test_extract_single_paper_success(self, extractor):
        ...
        with patch.object(extractor, '_get_paper_content', 
                         return_value=("Long content text", "tex")):
            with patch.object(extractor, '_llm_service_extract',
...
class TestTeXHTMLExtraction:
    ...
    def test_extract_tex_content_success(self, mock_harvester_class, extractor):
        ...
        mock_harvester = Mock()
        mock_harvester.fetch_tex_source.return_value = r"\documentclass{article}\begin{document}Content\end{document}"
    ...
    def test_extract_html_content_success(self, mock_harvester_class, extractor):
        ...
        mock_harvester = Mock()
        mock_harvester.fetch_html_source.return_value = "<html><body>Content</body></html>"
        mock_harvester_class.return_value = mock_harvester
