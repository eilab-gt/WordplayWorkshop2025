...
class TestExporter:
    ...
    def test_metadata_generation(self, sample_config, temp_dir):
        ...
        papers_df = pd.DataFrame(
            {"title": ["Paper 1", "Paper 2", "Paper 3"], "year": [2022, 2023, 2024]}
        )
    ...
    def test_create_readme(self, sample_config, temp_dir):
        ...
        papers_df = pd.DataFrame(
            {"title": ["Paper 1", "Paper 2"], "source_db": ["arxiv", "google_scholar"]}
        )

        extraction_df = pd.DataFrame(
            {"screening_id": ["SCREEN_0001"], "llm_family": ["GPT-4"]}
        )
    ...
    def test_export_formats(self, sample_config, temp_dir):
        ...
        papers_df = pd.DataFrame(
            {"title": ["Paper 1"], "year": [2024], "abstract": ["Test abstract"]}
        )

        extraction_df = pd.DataFrame(
            {"screening_id": ["SCREEN_0001"], "venue_type": ["conference"]}
        )
