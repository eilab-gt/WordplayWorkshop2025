...
def cli(ctx, config, log_level):
    ...
    console.print(f"[bold green]Literature Review Pipeline[/bold green]")
...
def harvest(ctx, query, sources, max_results, parallel, output):
    ...
    if output:
        ...
        output_path = config.raw_papers_path
...
def prepare_screen(ctx, input_file, output, asreview):
    ...
    if input_file:
        ...
        input_path = config.raw_papers_path
    ...
    if output:
        ...
        output_path = config.screening_progress_path
    
    screening_df = screen_ui.prepare_screening_sheet(
        df, 
        output_path=output_path,
        include_asreview=asreview
    )
...
def extract(ctx, input_file, output, parallel, skip_llm, skip_tagging):
    ...
    if input_file:
        ...
        input_path = config.screening_progress_path
    ...
    if not skip_tagging:
        ...
        if len(failure_summary) > 0:
            ...
            for _, row in failure_summary.head(5).iterrows():
                console.print(f"  {row['failure_mode']}: {row['count']} papers ({row['percentage']:.1f}%)")
...
def visualise(ctx, input_file, output_dir, show):
    ...
    if input_file:
        ...
        input_path = config.extraction_results_path
