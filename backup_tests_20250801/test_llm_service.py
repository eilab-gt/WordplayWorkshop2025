...
from src.lit_review.llm_service import AVAILABLE_MODELS, app
...
class TestLLMService:
    ...
    def test_models_endpoint_with_keys(self, client):
        ...
        with patch.dict(
            os.environ, {"GEMINI_API_KEY": "test-key", "OPENAI_API_KEY": "test-key"}
        ):
            response = client.get("/models")
            assert response.status_code == 200
            models = response.json()
    ...
    def test_extract_invalid_model(self, client):
        ...
        response = client.post(
            "/extract", json={"text": "Test text", "model": "invalid-model"}
        )
    ...
    def test_extract_no_api_key(self, client):
        ...
        with patch.dict(os.environ, {}, clear=True):
            response = client.post(
                "/extract", json={"text": "Test text", "model": "gemini/gemini-pro"}
            )
    ...
    def test_extract_request_validation(self, client):
        ...
        response = client.post("/extract", json={"model": "gemini/gemini-pro"})
        assert response.status_code == 422

        # Invalid temperature
        response = client.post(
            "/extract",
            json={"text": "Test", "temperature": 3.0},  # Out of range
        )
        assert response.status_code == 422
...
class TestEdgeCases:
    ...
    def test_extract_empty_text(self, client):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "test-key"}):
            response = client.post(
                "/extract", json={"text": "", "model": "gemini/gemini-pro"}
            )
            assert response.status_code == 200
    ...
    def test_extract_very_long_text(self, client):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "test-key"}):
            response = client.post(
                "/extract", json={"text": long_text, "model": "gemini/gemini-pro"}
            )
            assert response.status_code == 200
    ...
    def test_extract_with_json_embedded_in_text(self, mock_completion, client):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "test-key"}):
            response = client.post(
                "/extract", json={"text": "Test", "model": "gemini/gemini-pro"}
            )
